...
{
	q31_t* inbuf = (q31_t*) param0.GetBuffer();
	q31_t* outbuf = (q31_t*) param1.GetBuffer();
	for(INT32 i=0; i < param1.GetSize(); i++)
	{
		uint32_t sign=arm_recip_q31(inbuf[i], &outbuf[i], (q31_t*) armRecipTableQ31);
...
void VectorFuncs::VectorScale_Nat( CLR_RT_TypedArray_INT32 param0, INT32 param1, CLR_RT_TypedArray_INT32 param2, HRESULT &hr )
{
	...
	arm_scale_q31((q31_t*) param0.GetBuffer(), param1, 0, (q31_t*) param2.GetBuffer(), param0.GetSize());
}
