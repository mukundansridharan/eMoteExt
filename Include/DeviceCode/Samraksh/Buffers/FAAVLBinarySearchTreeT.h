 ...
 */
//#include "AVLBinarySearchTreeT.h2"
...
public:
	...
	void DeAllocateNode2(FAAVLBSTElement<T> * node){
	...
	FAAVLBSTElement<T>* AllocateNode3(){
	...
	FAAVLBSTElement<T>* AllocateNode2(const T &_data){
		FAAVLBSTElement<T>* p = this->AllocateNode3();
		if(p) p->data = _data;
		return p;
	}
	...
	bool Insert(const T &_data){
		//if(!(&_data){}
		FAAVLBSTElement<T>* newNode = this->AllocateNode2(_data);
		if(newNode){
